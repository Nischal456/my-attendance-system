...
export default function Workspace({ user, canAccessHub }) {
    ...
    const handleSetupBiometrics = async () => {
        ...
        try {
            ...
            if (verifyRes.ok) {
                toast.success("Face ID / Biometrics setup successful!", { id: 'bio-success' });
                setProfileUser(prev => ({ ...prev, passkeys: [{}] })); // Mock passkey entry to update UI
                setIsDropdownOpen(false);
            } else {
            ...
            if (error.name === 'NotAllowedError') {
                toast.error("Biometrics setup cancelled.");
            } else {
                toast.error(error.message || "Device does not support biometrics.");
            }
        } finally {
    ...
    const handleRemoveBiometrics = async () => {
        setIsRegisteringBiometrics(true);
        try {
            const res = await fetch("/api/auth/webauthn/remove-face-id", { method: "DELETE" });
            if (res.ok) {
                toast.success("Face ID removed successfully");
                setProfileUser(prev => ({ ...prev, passkeys: [] }));
                setIsDropdownOpen(false);
            } else {
                throw new Error("Failed to remove Face ID");
            }
        } catch (error) {
                                            ...
                                            {isDropdownOpen && (
                                                ...
                                                >
                                                    ...
                                                    <div className="p-2 space-y-1 bg-white/50">
                                                        ...
                                                        {/* Face ID Toggle */}
                                                        {profileUser?.passkeys?.length > 0 ? (
                                                            <button onClick={handleRemoveBiometrics} disabled={isRegisteringBiometrics} className="w-full text-left">
                                                                <motion.div
                                                                    whileHover={{ x: 4, backgroundColor: "rgba(255, 255, 255, 1)" }}
                                                                    className="group flex items-center gap-3 px-4 py-3 rounded-2xl cursor-pointer transition-all hover:shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)]"
                                                                >
                                                                    <div className="flex items-center justify-center w-9 h-9 rounded-xl bg-slate-100 text-slate-500 group-hover:bg-rose-500 group-hover:text-white transition-colors duration-300">
                                                                        {isRegisteringBiometrics ? <LucideLoader2 size={18} className="animate-spin" /> : <Trash2 size={18} strokeWidth={2.5} />}
                                                                    </div>
                                                                    <span className="text-sm font-semibold text-slate-600 group-hover:text-slate-900">Remove Face ID</span>
                                                                </motion.div>
                                                            </button>
                                                        ) : (
                                                            <button onClick={handleSetupBiometrics} disabled={isRegisteringBiometrics} className="w-full text-left">
                                                                <motion.div
                                                                    whileHover={{ x: 4, backgroundColor: "rgba(255, 255, 255, 1)" }}
                                                                    className="group flex items-center gap-3 px-4 py-3 rounded-2xl cursor-pointer transition-all hover:shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)]"
                                                                >
                                                                    <div className="flex items-center justify-center w-9 h-9 rounded-xl bg-slate-100 text-slate-500 group-hover:bg-blue-500 group-hover:text-white transition-colors duration-300">
                                                                        {isRegisteringBiometrics ? <LucideLoader2 size={18} className="animate-spin" /> : <Fingerprint size={18} strokeWidth={2.5} />}
                                                                    </div>
                                                                    <span className="text-sm font-semibold text-slate-600 group-hover:text-slate-900">Set up Face ID</span>
                                                                </motion.div>
                                                            </button>
                                                        )}
...
export async function getServerSideProps(context) {
    ...
    try {
        ...
        // All users are allowed to access their personal workspace now.
        return { props: { user: JSON.parse(JSON.stringify(user)), canAccessHub } };
