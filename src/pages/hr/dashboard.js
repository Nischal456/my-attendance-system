...
export default function HRDashboard({ user, initialAttendance, initialLeaveRequests, allUsers }) {
  ...
  const [activeView, setActiveView] = useState('attendance'); // 'attendance' or 'leaves'
  ...
  return (
    <>
      {isLeaveModalOpen && currentLeaveItem && ( <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4">{/* ... Leave Modal ... */}</div> )}
      {isDeleteModalOpen && ( <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4">{/* ... Delete Modal ... */}</div> )}
      ...
      <div className="min-h-screen bg-gray-50 p-6">
        <div className="max-w-7xl mx-auto">
            ...
            <div className="border-b border-gray-200">
                <nav className="-mb-px flex space-x-6" aria-label="Tabs">
                    <button
                        ...
                        className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm flex items-center gap-2 ${activeView === 'attendance' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
                    >
                        <Calendar size={16} />
                    ...
                    <button
                        ...
                        className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm flex items-center gap-2 ${activeView === 'leaves' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
                    >
                        <Briefcase size={16} />
                        Leave Management
                    </button>
          ...
          {/* --- NEW: Conditional Rendering of Views --- */}
          ...
          {activeView === 'attendance' && (
            <div className="space-y-8">
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm overflow-x-auto border">
...
// --- UPDATED: getServerSideProps now fetches all necessary data ---
export async function getServerSideProps(context) {
  ...
  try {
    ...
    const [allAttendance, leaveRequests, allUsers] = await Promise.all([
        Attendance.find({}).populate("user", "name role phoneNumber avatar").sort({ checkInTime: -1 }),
        LeaveRequest.find({}).populate('user', 'name role').sort({ status: 1, createdAt: -1 }),
