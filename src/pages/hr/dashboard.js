...
import { Send, Trash2, AlertTriangle, LogOut, Check, X as XIcon, MessageSquare, UserPlus, Calendar, Briefcase } from 'react-feather';
...
export default function HRDashboard({ user, initialAttendance, initialLeaveRequests, allUsers }) {
  ...
  const [activeView, setActiveView] = useState('leaves');
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      // Correctly update both state arrays to refresh the UI instantly
  ...
  const openLeaveModal = (request) => {
    setCurrentLeaveItem(request);
  ...
  const handleManageLeave = async (newStatus) => {
    ...
    setError('');
    try {
      const res = await fetch('/api/hr/manage-leave', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          leaveId: currentLeaveItem._id,
          status: newStatus,
      ...
      {isLeaveModalOpen && currentLeaveItem && (
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4">
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-lg animate-fade-in-up">
            <h3 className="text-xl font-semibold mb-4 text-gray-800">Manage Leave Request</h3>
            <div className="text-sm space-y-2 p-4 bg-gray-50 rounded-md mb-4 border">
              <p><strong>Employee:</strong> {currentLeaveItem.user.name}</p>
              <p><strong>Dates:</strong> {formatDateForLeave(currentLeaveItem.startDate)} to {formatDateForLeave(currentLeaveItem.endDate)}</p>
              <p><strong>Type:</strong> {currentLeaveItem.leaveType}</p>
              <p><strong>Reason:</strong> {currentLeaveItem.reason}</p>
            </div>
            <div><label htmlFor="hrComments" className="block text-sm font-medium text-gray-700">Add or Edit HR Comments</label><textarea id="hrComments" value={hrComments} onChange={(e) => setHrComments(e.target.value)} rows="3" className="w-full mt-1 p-2 border border-gray-300 rounded-md shadow-sm"></textarea></div>
          ...
          {activeView === 'attendance' && (
            <div className="space-y-8">
                <div className="bg-white p-6 rounded-xl shadow-sm">
                    ...
                    <div className="flex-1"><label htmlFor="role-filter" className="block text-sm font-medium text-gray-700 mb-1">Filter by Role</label><select id="role-filter" value={selectedRole} onChange={(e) => { setSelectedRole(e.target.value); setSelectedUser("All"); }} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm"><option value="All">All Roles</option><option value="Staff">Staff</option><option value="Intern">Intern</option><option value="Manager">Manager</option><option value="Project Manager">Project Manager</option></select></div>
                    <div className="flex-1"><label htmlFor="user-filter" className="block text-sm font-medium text-gray-700 mb-1">Filter by Employee</label><select id="user-filter" value={selectedUser} onChange={(e) => setSelectedUser(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm"><option value="All">All Employees</option>{allUsers.filter((u) => selectedRole === "All" || u.role === selectedRole).map((u) => (<option key={u._id} value={u._id}>{u.name}</option>))}</select></div>
                    </div>
                </div>
                <div className="bg-white p-6 rounded-xl shadow-sm">
                    <h2 className="text-xl font-semibold text-gray-800 mb-4">Filtered Summary</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div className="bg-green-50 p-4 rounded-lg border border-green-100"><h3 className="text-sm font-medium text-green-800 mb-1">Total Logged Time</h3><p className="text-2xl font-bold text-green-600">{formatDuration(totalWorkSeconds)}</p></div><div className="bg-blue-50 p-4 rounded-lg border border-blue-100"><h3 className="text-sm font-medium text-blue-800 mb-1">Days Meeting 6-Hour Target</h3><p className="text-2xl font-bold text-blue-600">{daysMeetingTarget} <span className="text-base font-normal text-gray-500">/ {completedDays.length}</span></p></div></div>
                </div>
                <div className="bg-white p-6 rounded-xl shadow-sm overflow-x-auto">
                    <div className="flex justify-between items-center mb-4"><h2 className="text-xl font-semibold text-gray-800">Attendance History</h2><p className="text-sm text-gray-500">Showing {filteredAttendance.length} of {initialAttendance.length} records</p></div>
