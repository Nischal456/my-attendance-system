...
import {
  Loader2, Lock, Eye, EyeOff, ArrowRight, CheckCircle2, ShieldCheck, ChevronLeft, LockKeyhole
} from "lucide-react";
...
export default function ResetPasswordPage() {
  ...
  const showSuccessToast = () => {
    ...
    toast.custom((t) => (
      ...
      >
        ...
        <div>
          <h3 className="font-bold text-slate-800 text-sm">Password Updated Successfully</h3>
          <p className="text-[11px] font-medium text-slate-500">Redirecting to login portal...</p>
        </div>
      </motion.div>
    ), { id: 'reset-success', duration: 2500, position: "top-center" });
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (password !== confirmPassword) {
      toast.error("Passwords do not match.", { id: 'match-err', style: { borderRadius: '16px', fontWeight: 'bold' } });
      return;
    }
    if (password.length < 6) {
      toast.error("Password must be at least 6 characters long.", { id: 'len-err', style: { borderRadius: '16px', fontWeight: 'bold' } });
      return;
    }
    if (!token) {
      toast.error("Invalid or missing reset token. Please request a new link.", { id: 'token-err', style: { borderRadius: '16px', fontWeight: 'bold' } });
      return;
    }
  ...
  const brandName = "Password Reset".split("");
          ...
          >
            ...
            <div className="flex flex-col items-center justify-center mb-8">
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-center">
                <h2 className="text-3xl font-black text-slate-800 tracking-tight mb-2">Create New Password</h2>
            ...
            {resetSuccess ? (
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="space-y-6 text-center">
                <div className="p-6 rounded-[2rem] bg-emerald-50/80 border border-emerald-100 text-emerald-900 space-y-3">
                  ...
                  <h3 className="font-extrabold text-base">Password Reset Complete</h3>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="space-y-2 group/input">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400 ml-4 group-focus-within/input:text-emerald-600 transition-colors">New Password</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="••••••••"
                      required
                      className="w-full bg-slate-50/50 border-2 border-slate-100 rounded-[1.5rem] py-4 pl-14 pr-12 text-slate-800 font-bold focus:outline-none focus:border-emerald-400 focus:bg-white focus:shadow-lg focus:shadow-emerald-100 transition-all placeholder:text-slate-300 text-sm"
                    />
                ...
                <div className="space-y-2 group/input">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400 ml-4 group-focus-within/input:text-emerald-600 transition-colors">Confirm New Password</label>
                  <div className="relative">
                    <div className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within/input:text-emerald-500 transition-colors"><Lock size={20} /></div>
                    <input
                      ...
                      className="w-full bg-slate-50/50 border-2 border-slate-100 rounded-[1.5rem] py-4 pl-14 pr-12 text-slate-800 font-bold focus:outline-none focus:border-emerald-400 focus:bg-white focus:shadow-lg focus:shadow-emerald-100 transition-all placeholder:text-slate-300 text-sm"
                    />
                    <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-5 top-1/2 -translate-y-1/2 text-slate-400 p-1 hover:text-emerald-600 transition-colors">
                ...
                >
                  ...
                  <div className="relative flex items-center justify-center gap-3">
                    {loading ? <Loader2 size={20} className="animate-spin" /> : <><span>Reset Password</span><ArrowRight size={20} className="group-hover:translate-x-1 transition-transform" /></>}
                  </div>
                </button>
              </form>
            )}
