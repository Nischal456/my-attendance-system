...
const PersonalTaskModal = ({ onClose, onTaskCreated }) => {
    ...
    {/* Form Fields Section with Floating Labels */}
    <div className="space-y-8 pt-2">
        {/* Task Title Floating Label Input */}
        <div className="relative">
            <input
                ...
                // The 'peer' class is crucial for the animation
                className="peer block w-full appearance-none rounded-md border-slate-300 bg-transparent px-3 py-2.5 text-sm text-slate-900 shadow-sm focus:border-indigo-600 focus:outline-none focus:ring-0"
                placeholder=" " // The space is necessary for the placeholder-shown selector to work
                required
            />
            <label
                htmlFor="title"
                className="absolute left-3 top-2.5 z-10 origin-[0] -translate-y-4 scale-75 transform text-sm text-slate-500 duration-300 peer-placeholder-shown:translate-y-0 peer-placeholder-shown:scale-100 peer-focus:-translate-y-4 peer-focus:scale-75 peer-focus:text-indigo-600"
            >
                Task Title <span className="text-red-500">*</span>
            </label>
        </div>

        {/* Task Description Floating Label Textarea */}
        <div className="relative">
            <textarea
                id="description"
                name="description"
            ...
            <label
                ...
                className="absolute left-3 top-2.5 z-10 origin-[0] -translate-y-4 scale-75 transform text-sm text-slate-500 duration-300 peer-placeholder-shown:translate-y-0 peer-placeholder-shown:scale-100 peer-focus:-translate-y-4 peer-focus:scale-75 peer-focus:text-indigo-600"
            >
                Description of your Task <span className="text-slate-400"></span>
            </label>
        </div>
    </div>

    {/* --- Error Display and Form Actions remain the same as the previous update --- */}
    ...
    <div className="mt-8 flex flex-col-reverse items-center gap-4 border-t border-slate-200 pt-5 sm:flex-row sm:justify-end">
        <button
            type="button"
            onClick={onClose}
            className="w-full justify-center rounded-md border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-sm hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 sm:w-auto"
        >
            Cancel
        </button>
        <button
...
const WorkHoursChartCard = () => {
    ...
    useEffect(() => {
        const fetchHours = async () => {
            ...
            try {
                const res = await fetch(`/api/dashboard/work-hours?year=${year}&month=${month}`);
                if (!res.ok) throw new Error('Could not fetch work hours.');
...
export async function getServerSideProps(context) {
    // FIX: Server-side imports moved here to prevent build errors
