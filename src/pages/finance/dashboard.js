...
// --- Helper Functions ---
const formatCurrency = (amount) => `Rs. ${new Intl.NumberFormat('en-IN', { maximumFractionDigits: 2 }).format(amount)}`;
...
const ActionCard = ({ onAction }) => (
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200/80">
        ...
        <div className="grid grid-cols-2 gap-4">
            ...
            <button onClick={() => onAction('Deposit')} className="bg-blue-50 text-blue-700 font-bold py-3 px-4 rounded-lg flex items-center justify-center gap-2 hover:bg-blue-100 transition-colors transform hover:scale-105"><Plus size={18}/> Bank Deposit</button>
            <button onClick={() => onAction('Withdrawal')} className="bg-yellow-50 text-yellow-700 font-bold py-3 px-4 rounded-lg flex items-center justify-center gap-2 hover:bg-yellow-100 transition-colors transform hover:scale-105"><Minus size={18}/> Bank Withdrawal</button>
        </div>
    </div>
);
...
const TransactionRow = ({ transaction, index, onRowClick }) => {
    const isIncome = transaction.type === 'Income' || transaction.type === 'Deposit';
    return (
        <div onClick={onRowClick} className="flex items-center p-3 hover:bg-slate-100/70 rounded-lg transition-colors cursor-pointer animate-fade-in-up" style={{ animationDelay: `${index * 40}ms` }}>
            ...
            <div className="flex-1 overflow-hidden"><p className="font-semibold text-slate-800 truncate">{transaction.title}</p><p className="text-xs text-slate-500">{transaction.category}</p></div>
            <div className="text-sm text-slate-500 mx-4 hidden sm:block">{formatDate(transaction.date)}</div>
            <div className={`text-md font-bold text-right ml-4 ${isIncome ? 'text-green-600' : 'text-red-600'}`}>{isIncome ? '+' : '-'}{formatCurrency(transaction.amount)}</div>
        </div>
    );
};
...
export default function FinanceDashboard({ user, allUsers, initialTransactions, initialBankAccount, initialNotifications }) {
  ...
  const [targetUser, setTargetUser] = useState(allUsers[0]?._id || '');
  ...
  const filteredTransactions = useMemo(() => { return transactions.filter(t => { const transactionDate = new Date(t.date); return transactionDate.getFullYear() === viewingYear && transactionDate.getMonth() === viewingMonth; }); }, [transactions, viewingMonth, viewingYear]);
  const summary = useMemo(() => { let totalIncome = 0, totalExpenses = 0; filteredTransactions.forEach(t => { if (t.type === 'Income') totalIncome += t.amount; else if (t.type === 'Expense') totalExpenses += t.amount; }); return { totalIncome, totalExpenses, netProfit: totalIncome - totalExpenses }; }, [filteredTransactions]);
  ...
  const handleSendNotification = async (e) => {
    e.preventDefault();
    if (!notificationContent.trim() || !targetUser) {
  ...
  const handleDownloadStatement = async () => {
    ...
    try {
        ...
        const summaryRows = [`\n\nSummary for ${summaryTitle}`, `Total Income:,"${formatCurrency(totalIncome)}"`, `Total Expenses:,"${formatCurrency(totalExpenses)}"`, `Net Profit/Loss:,"${formatCurrency(totalIncome - totalExpenses)}"`];
        ...
        link.setAttribute("download", `financial_statement_${reportYear}_${reportType === 'monthly' ? reportMonth + 1 : ''}.csv`);
  ...
  return (
    <>
      ...
      {viewingTransaction && (<TransactionDetailModal transaction={viewingTransaction} onClose={() => setViewingTransaction(null)} />)}
        ...
        <div className="relative z-10">
            ...
            <main className="p-6 sm:p-8 lg:p-10">
                <div className={`max-w-7xl mx-auto space-y-8 transition-all duration-500 ease-out ${isMounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}>
                    <div><h1 className="text-3xl font-bold text-slate-800">Financial Overview</h1><p className="text-slate-500 mt-1">Summary for <span className="font-semibold text-indigo-600">{monthOptions.find(m => m.value === viewingMonth)?.label} {viewingYear}</span></p></div>
                    {message && <div className="p-3 bg-green-50 text-green-700 rounded-lg flex items-center gap-2"><CheckCircle size={18}/><span>{message}</span></div>}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6"><StatCard title="Income this month" value={formatCurrency(summary.totalIncome)} icon={<TrendingUp className="text-green-600"/>} iconBgColor="bg-green-100" /><StatCard title="Expenses this month" value={formatCurrency(summary.totalExpenses)} icon={<TrendingDown className="text-red-600"/>} iconBgColor="bg-red-100" /><StatCard title="Monthly Net" value={formatCurrency(summary.netProfit)} icon={<DollarSign className="text-indigo-600"/>} iconBgColor="bg-indigo-100" /><StatCard title="Bank Balance" value={formatCurrency(bankAccount.balance)} icon={<CreditCard className="text-blue-600"/>} iconBgColor="bg-blue-100" /></div>
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8"><div className="lg:col-span-4 space-y-8 animate-fade-in-up"><ActionCard onAction={openModal} /><div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200/80"><h2 className="text-xl font-semibold text-slate-800 mb-4">Generate Statement</h2><div className="space-y-4"><div className="flex gap-4"><select value={reportType} onChange={e => setReportType(e.target.value)} className="w-full p-2 border rounded-lg bg-white"><option value="monthly">Monthly</option><option value="yearly">Yearly</option></select><select value={reportYear} onChange={e => setReportYear(parseInt(e.target.value))} className="w-full p-2 border rounded-lg bg-white">{yearOptions.map(y => <option key={y} value={y}>{y}</option>)}</select></div>{reportType === 'monthly' && (<div><label className="text-xs text-slate-500">Month</label><select value={reportMonth} onChange={e => setReportMonth(parseInt(e.target.value))} className="w-full p-2 border rounded-lg bg-white">{monthOptions.map(m => <option key={m.value} value={m.value}>{m.label}</option>)}</select></div>)}<button onClick={handleDownloadStatement} className="w-full bg-indigo-600 text-white font-bold py-3 rounded-lg flex items-center justify-center gap-2 hover:bg-indigo-700 transition-colors"><Download size={18}/>Generate & Download CSV</button></div></div><NotificationCard allUsers={allUsers} onSubmit={handleSendNotification} content={notificationContent} setContent={setNotificationContent} targetUser={targetUser} setTargetUser={setTargetUser} isSending={isSending} /></div><div className="lg:col-span-8 bg-white p-6 rounded-2xl shadow-sm border border-slate-200/80 animate-fade-in-up"><div className="flex flex-col sm:flex-row justify-between sm:items-center mb-4 gap-4"><h2 className="text-xl font-semibold text-slate-800">Transactions</h2><div className="flex gap-2"><select value={viewingMonth} onChange={e => setViewingMonth(parseInt(e.target.value))} className="p-2 border rounded-lg bg-white">{monthOptions.map(m => <option key={m.value} value={m.value}>{m.label}</option>)}</select><select value={viewingYear} onChange={e => setViewingYear(parseInt(e.target.value))} className="p-2 border rounded-lg bg-white">{yearOptions.map(y => <option key={y} value={y}>{y}</option>)}</select></div></div><div className="space-y-2">{filteredTransactions.map((t, index) => <TransactionRow key={t._id} transaction={t} index={index} onRowClick={() => setViewingTransaction(t)} />)}{filteredTransactions.length === 0 && <p className="text-center py-10 text-slate-500">No transactions found for this period.</p>}</div></div></div>
                </div>
            </main>
        </div>
      </div>
