...
import {
  ...
  CheckCircle2, ShieldCheck, ChevronLeft, MessageCircle, Sparkles, Fingerprint
} from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleBiometricLogin = async () => {
    if (!email) {
      toast.error("Please enter your email first to use Face ID", { id: 'bio-error' });
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        toast.error(options.message || "Biometric login failed");
      ...
      if (verificationRes.ok && verificationBody.verified) {
        ...
        toast.error(verificationBody.error || "Biometric Verification Failed");
      }
    } catch (err) {
      if (err.name === 'NotAllowedError') {
        toast.error("Biometrics cancelled", { id: 'bio-cancelled' });
      } else {
        toast.error("Device does not support biometrics or configuration error");
      }
      console.error(err);
    } finally {
      setBiometricLoading(false);
            ...
            >
              ...
              >
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  {/* Sleek Biometrics Separator & Button */}
                  <div className="pt-4 pb-2">
                    ...
                    <button
                      ...
                      className="group relative w-full overflow-hidden rounded-[1.25rem] bg-white border border-emerald-500/10 py-3.5 px-5 font-bold text-slate-600 shadow-[0_4px_12px_-4px_rgba(16,185,129,0.1)] hover:border-emerald-500/30 hover:bg-emerald-50/30 hover:shadow-[0_8px_20px_-6px_rgba(16,185,129,0.2)] hover:text-emerald-700 active:scale-95 transition-all duration-300 disabled:opacity-70 disabled:hover:scale-100"
                    >
                      ...
                      <div className="relative flex items-center justify-center gap-2.5">
                        {biometricLoading ? <Loader2 size={18} className="animate-spin text-emerald-500" /> : <>
                          <div className="p-1.5 rounded-lg bg-emerald-100/50 text-emerald-600 group-hover:bg-emerald-500 group-hover:text-white transition-colors duration-300">
